...
class RequestsPlus(util.UtilMixin, object):
    def __init__(self, apiBaseURL, contentType=MIME_TYPE_JSON, authZToken=None,
                 authZType=AUTHZ_TYPE_BEARER):
    ...
    def get(self, apiQueryURI, **kwargs):
        ...
        if not response.ok:
            raise RuntimeError(
                'Error {response.status_code} "{response.reason}" for request: {apiQueryURI}'
                    .format(**locals()))
